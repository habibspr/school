...
<div class="container">
    ...
    // studentid
    $studentid = '';
    $Student_Id_Result = mysqli_query($link, "SELECT st_id FROM student_info, st_session, st_class WHERE ss_st_id = st_id AND ss_class_id = sc_id AND sc_id = '$class' AND ss_roll='$roll' AND ss_year = '$session' AND st_status = '0' " );
    ...
    <div class="alert alert-success fade in">
        ...
        <strong>Notice!</strong> ........No Roll Found!
    </div>
    <?php die(); } 
   ...
   while($row = mysqli_fetch_array($Merit_Position_Result)){
    ...
    <div class="col-sm-12 row table-responsive">
        ...
        <table class="table table-bordered table-condensed table-striped table-hover">
            ...
            <?php
        $result = mysqli_query($link, "SELECT exm_sub_code, exm_sub_name, mrk_creative_full_mark, mrk_objective_full_mark, mrk_practical_full_mark, mrk_creative, mrk_objective, mrk_practical, mrk_subject_id, mrk_extra, mrk_grade, mrk_gp  FROM exm_subject_mst, exm_marks WHERE exm_subject_mst.exm_sub_id=exm_marks.mrk_subject_id AND exm_marks.mrk_exam_id=$exam AND exm_marks.mrk_st_id=$studentid ORDER BY exm_marks.mrk_subject_id ASC");
